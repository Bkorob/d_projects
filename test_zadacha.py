...
class LinkedList: # связанный список
    ...
    def insert(self, index, data): # вставка элемента в связанный список
        new_node = Node(data) # создаём новый объект который будем вставлять
        curr_node = self.head # присваиваем указателю голову
        counter = 0 # заводим счётчик
        while curr_node.get_next() != None: # пока следующий от указателя узел не НАН
            if index == 0: # доп.условие если указанный индекс == 0 
                self.push_front(data) # вставляем элемент с помощью метода push_front
                return # возвращаем получившийся список
            elif counter + 1  == index: # второе доп.условие: как только дошли до предыдущего от 
            # инвекс элемента
                next_curr_item = curr_node.get_next() # заводим доп переменную, куда прячем 
                # текущий узел, равный индексу 
                curr_node.set_next(new_node) # присваиваем следующему от указателя узлу новый узел
                new_node.set_next(next_curr_item) # связываем присовенный узел со спрятанным в переменную
                return # возвращаем получившийся список
            counter += 1 # иначе добавляем счётчику единицу
            curr_node = curr_node.get_next() # передвигаем указатель на один узел вперед
        raise IndexError('Index is out of range') # если совпадения с индексом не было, вызываем исключение
    
    def remove(self, index): # удаление элемента по индексу
        curr_node = self.head # присваиваем указателю голову
        counter = 0 # заводим счётчик 
        while curr_node.get_next() != None: # пока следующий от указателя узел не НАН
            if index == 0: # доп.условие: если индекс == 0 
                self.remove_front() # вызываем метод remove_front для удаления гоовы
                return # возращаем получившийся список
            elif counter + 1 == index # второе доп.условие: как только дошли до предыдущего от
                        # индекса элемента
                curr_node.set_next(curr_node.get_next().get_next()) # указываем следующим
                # элементом, тот что после индекса
                return # возращаем получившися список
            counter += 1 # иначе добавляем к счётчику +1
            curr_node = curr_node.get_next() # указатель передвигаем на элемент вперед
        raise IndexError('Index is out of range') # если индекс не совпал вызываем исключение
    
    def reverse(self): # разворачиваем список
        prev = None # заводим переменнную для предыдущего эл-та
        next = None # заводим переменнную для следующего эл-та
        curr_node = self.head # присваиваем указателю голову
        while curr_node != None: # пока текущий узел не НАН
            next = curr_node.get_next() # перемещаем вперед на один
            curr_node.set_next(prev) # связываем указатель с предыдущим эл-том(1раз это НАН)
            prev = curr_node # предыдущим становится текущий
            curr_node = next # указатель передвигается на место следующего
        self.head = prev # в конце цикла, когда указатель НАН, голову присваивам предыдущему
